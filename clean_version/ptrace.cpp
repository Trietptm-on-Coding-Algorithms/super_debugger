 ...
 *
 */
void* Ptrace::get_instr_ptr( pid_t pid ) {
 ...
 *
 */
void Ptrace::set_instr_ptr( pid_t pid, void* addr ) {
 ...
 *
 */
void* Ptrace::get_base_ptr( pid_t pid ) {
 ...
 *
 */
void* Ptrace::get_stack_ptr( pid_t pid ) {
 ...
 *
 */
void* Ptrace::get_ret_addr( pid_t pid, void* stack_pointer ) {
 ...
 *
 */
long Ptrace::get_data( pid_t pid, void* addr ) {
 ...
 *
 */
void Ptrace::set_data( pid_t pid, void* addr, void* data ) {
 ...
 *
 */
void Ptrace::getRegisters( pid_t pid ) {
 ...
 *
 */
void Ptrace::printRegisters( pid_t pid ) {
	cerr << "\n";
 ...
 *
 */
void Ptrace::set_brkpt( pid_t pid, void* addr, bool is_new_brkpt ) {			
 ...
 *
 */
void Ptrace::set_watchpt( pid_t pid , string symbol ) {
	map< string, ObjDump::symbol_data >& symbol_table = all_functs[curr_function].symbol_table;
	ObjDump::symbol_data sym = symbol_table[symbol];
	
	Ptrace::watchpt_info storage;
 ...
 *
 */
void Ptrace::handle_brkpt( pid_t pid, void* instr_ptr, void* data ) {
 ...
 *
 */
bool Ptrace::is_brkpt( pid_t pid ) {
 ...
 *
 */
bool Ptrace::is_num( string &input ) {
 ...
 *
 */
bool Ptrace::is_brkpt_step( pid_t pid ) {
	void* instr_addr = get_instr_ptr( pid );
 ...
 *
 */
bool Ptrace::input_next( string &input ) {	
	for ( int i = 0; i < 4; ++i )
 ...
 *
 */
bool Ptrace::input_break( string &input ) {
 ...
 *
 */
bool Ptrace::input_print( string &input ) {
 ...
 *
 */
bool Ptrace::input_rm( string &input ) {
 ...
 *
 */
bool Ptrace::input_watch( string &input ) {
 ...
 *
 */
bool Ptrace::input_exit( string &input ) {
	for ( int i = 0; i < 4; ++i )
 ...
 *
 */
void Ptrace::insert_brkpt_helper( void* addr, int line_num, bool direct_hit ) {
 ...
 *
 */
void Ptrace::find_print_brk_loc( string &str, bool is_line_num ) {
	if ( is_line_num ) {
 ...
 *
 */
void Ptrace::rm_brkpt_helper( pid_t pid, int line_num ) {
	...
	if ( (line_it = line_to_addr.find(line_num)) != line_to_addr.end() ) {
		...
		if ( (it = helper_brkpt.find(line_it -> second)) != helper_brkpt.end() ) {
			...
			if ( (all_brkpts_it = all_brkpts.find( line_it -> second )) != all_brkpts.end() ) {
				Ptrace::brkpt_info temp = all_brkpts_it -> second;
 ...
 *
 */
void Ptrace::find_print_rm_loc( pid_t pid, string &str, bool is_line_num ) {
	int line_num = -1;
 ...
 *
 */
void Ptrace::handle_brk_or_rm( pid_t pid, string input_cmd, bool in_break /* true: is setting brkpt */ ) {
 ...
 *
 */
void Ptrace::handle_rm_watch(  pid_t pid, string input_cmd ) {
	input_cmd.erase( 0, input_cmd.find(" ") + 1 );
	trim( input_cmd );
	map< string, ObjDump::symbol_data >& symbol_table = all_functs[curr_function].symbol_table;
 ...
 *
 */
void Ptrace::insert_watchpt_batch( pid_t pid ) {
 ...
 */
void Ptrace::insert_brkpt_batch( pid_t pid ) {
 ...
 */
void Ptrace::brkpt_handler( pid_t pid, bool in_step ) {
	void* instr_addr = get_instr_ptr( pid );
	...
	Ptrace::brkpt_info curr_brkpt = all_brkpts[new_instr_addr];
 ...
 * 
 */
void Ptrace::delete_symbols( pid_t pid, string function ) {
	void* base_ptr = all_functs[function].base_ptr_addr;
	map< string, ObjDump::symbol_data >& symbol_table = all_functs[function].symbol_table;
 ...
 * 
 */
void Ptrace::fill_symbols( pid_t pid, string function ) {
 ...
 *
 */
void Ptrace::build_symbol_table( pid_t pid, string function ) {
 ...
 *
 */
void Ptrace::switch_symbol_table( pid_t pid, int line_num ) {
 ...
 *
 */
void Ptrace::set_brkpt_main( pid_t pid ) {
 ...
 *
 */
void Ptrace::print_char_sym( pid_t pid, long text_addr ) {
	char c[8];
	long char_arr;
	bool flag = true;
	int idx = 0;
 ...
 *
 */
void Ptrace::print_types( pid_t pid, long data, int type_idx ) {
 ...
 *
 */
void Ptrace::print_symbol( pid_t pid, string &symbol ) {
	map< string, ObjDump::symbol_data >& symbol_table = all_functs[curr_function].symbol_table;
	
	auto it = symbol_table.begin();
	if ( (it = symbol_table.find(symbol)) != symbol_table.end() ) {
		ObjDump::symbol_data sym = it -> second;
		long data = get_data( pid, (void*)sym.addr );
 ...
 *
 */
void Ptrace::handle_print( pid_t pid, string input_cmd ) {
	input_cmd.erase( 0, input_cmd.find(" ") + 1 );
	trim( input_cmd );
 ...
 *
 */
bool Ptrace::check_watchpts( pid_t pid ) {
	int flag = true;
	map< string, ObjDump::symbol_data >& symbol_table = all_functs[curr_function].symbol_table;

	for ( auto it = all_watchpts[curr_function].begin(); it != all_watchpts[curr_function].end(); ++it ) {
		ObjDump::symbol_data sym = symbol_table[it -> first];
		long data = get_data( pid, (void*)sym.addr );
 ...
 *
 */
void Ptrace::set_temp_watchpt( pid_t pid, string input_cmd ) {
	input_cmd.erase( 0, input_cmd.find(" ") + 1 );
	trim( input_cmd );
	map< string, ObjDump::symbol_data >& symbol_table = all_functs[curr_function].symbol_table;
	auto it = symbol_table.begin();
 ...
 *
 */
bool Ptrace::step_handler( pid_t pid ) {
	void* instr_addr = get_instr_ptr( pid );
 ...
 *
 */
bool Ptrace::step_watch( pid_t pid ) {
	void* instr_addr = get_instr_ptr( pid );
 ...
 *
 */
void Ptrace::prog_active( pid_t pid, bool is_init_stop ) {
	if ( is_init_stop ) {
		printIntro();
 ...
 *
 */
int Ptrace::main_debugger( int argc, char* argv[] ) {
	curr_function = "main";
	...
	if ( parent ) {
		...
		if ( WIFSTOPPED(status) ) {
			if ( WSTOPSIG(status) == SIGTRAP ) {		
				...
				// restore main
				Ptrace::brkpt_info main_brkpt = all_brkpts[instr_addr - 1];
		...
		if ( !ptrace(PTRACE_TRACEME, 0, NULL, NULL) && "Cannot trace child process for debugging" );
