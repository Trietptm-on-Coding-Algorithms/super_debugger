...
/*
void print_watchpt( pid_t pid, string symbol, watchpt_info watch, int size ) {
	cerr << "\nWatchpoint '" << it -> first << "', at " << filename << ":" << addr_to_line[instr_ptr] << "\n";
	cerr << "Former value = ";
...
void prog_active( pid_t pid, bool is_init_stop ) {
	if ( is_init_stop ) {
		...
		if ( !filled_sym_table )				// fill symbol table the first time we stop
			fill_symbols( pid );
		...
		if ( all_watchpts.size() ) {				// continue through watchpoint
			...
			if ( is_brkpt_step(pid) ) 		// must be stepping through breakpt
...
int main_debugger( int argc, char* argv[] ) {
	...
	if ( parent ) {
		...
		/*
		// wait again for our breakpoint we inserted to build symbol table
		waitpid( parent, &status, 0 );
