...
// symbol (variable name), pair of [ memory addr, type ]
map< string, pair<void*, int> > symbol_table;	

// all addresses and types of active watchpoints
map< void*, int > watchpt_addr;					// memory addr, type
...
void set_watchpt( pid_t pid , string symbol ) {
	pair< void*, int > symbol_info = symbol_table[symbol];
	watchpt_info storage;
	long data = get_data( pid, symbol_info.first );
	storage.data = data;
	storage.type = symbol_info.second;
	storage.times_hit = 0;
	all_watchpts[symbol] = storage;
	
	// also store the address 
	watchpt_addr[symbol_info.first] = symbol_info.second;
}
...
void fill_symbols( pid_t pid ) {
	...
	for ( auto it = all_symbols.begin(); it != all_symbols.end(); ++it ) {
		...
		if ( symbol_table.find(it -> first) == symbol_table.end() ) {
			...
			cerr << "Something went wrong, debugger terminating\n";
			kill( pid, SIGKILL ); 
		}
	}
}
...
void prog_active( pid_t pid, bool is_init_stop ) {
	if ( is_init_stop ) {
		...
		// we will be at a valid line number here
		// make sure step_handler does not skip due to same line stuff
