...
// inserts all the breakpoints after user continues
...
// implimented similar to gdb
// handle stepping --- returns true on error
bool step_handler( pid_t pid ) {
	...
	for ( int i = 0; i < x86_code.size(); ++i ) {
		if ( x86_code[i].lineNum == former_linenum ) {
			idx = i;
			break;
		}
	}
	...
	while ( 1 ) {
		instr_addr = get_instr_ptr( pid );
		
		if ( still_in_addr(instr_addr, idx) )
...
// is_init_stop: true = when the program first stops to let the user input breakpoints
	...
	while ( getline(cin, input_cmd) ) {
		if ( !input_cmd.compare("run") || !input_cmd.compare("r") ) {
			...
			if ( is_init_stop ) {
				...
				if ( is_brkpt(pid) ) {
					// getting the current line number so we know how to step
					void* instr_addr = get_instr_ptr( pid );
					void* new_instr_addr = instr_addr - 1;
					former_linenum = addr_to_line[new_instr_addr];
				}
				else {
					void* instr_addr = get_instr_ptr( pid );
					former_linenum = addr_to_line[instr_addr];
				}
