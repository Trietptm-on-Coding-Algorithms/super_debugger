...
// flag to check if we sucessfully filled in symbol table
bool filled_sym_table;
...
void find_print_brk_loc( string &str, bool is_line_num ) {
	if ( is_line_num ) {
		...
		if ( (it = line_to_addr.find(line_num)) != line_to_addr.end() ) {
			...
			if ( helper_brkpt.find( (it -> second)[idx] ) == helper_brkpt.end() ) {
				++brkpt_num;
				cerr << "Breakpoint " << brkpt_num << " set at address: '" << (it -> second)[idx] << "', file: '";
				cerr << filename << "', line: '" << line_num << "'\n";
				helper_brkpt[ (it -> second)[idx] ] = brkpt_num;
				brkpt_idx[ brkpt_num ] = (it -> second)[idx];
				brkpt_batch.insert( (it -> second)[idx] );
			}
			else {
				cerr << "Breakpoint already set at this address, breakpoint request ignored\n";
			}
			...
			if ( helper_brkpt.find( (it -> second)[idx] ) == helper_brkpt.end() ) {
				...
				cerr << "Breakpoint " << brkpt_num << " set at address: '" << (it -> second)[idx] << "', file: '";
				cerr << filename << "', line: '" << addr_to_line[ (it -> second)[idx] ] << "'\n";
		...
		if ( (line_it = line_to_addr.find(line_num)) != line_to_addr.end() ) {
			...
			if ( (it = helper_brkpt.find( (line_it -> second)[idx] )) != helper_brkpt.end() ) {
				...
				if ( (all_brkpts_it = all_brkpts.find( (line_it -> second)[idx] )) != all_brkpts.end() ) {
					...
					if ( instr_addr == (line_it -> second)[idx] ) 
						reset_brkpt = false;
...
bool check_watchpts( pid_t pid ) {
	...
	for ( auto it = all_watchpts.begin(); it != all_watchpts.end(); ++it ) {
		...
		if ( data != former_data ) {
			flag = false;
			void* instr_ptr = get_instr_ptr( pid );
			int line_num = addr_to_line[instr_ptr];
			cerr << "\nWatchpoint '" << it -> first << "', at " << filename << ":" << line_num << "\n";
			cerr << "Former value = ";
...
void prog_active( pid_t pid, bool is_init_stop ) {
	...
	if ( is_init_stop ) {
		set_brkpt( pid, break_main_addr, true );			// set breakpoint at main to build symbol table
		ptrace( PTRACE_CONT, pid, NULL, NULL );
	}
	else {
	...
	if ( parent ) {
		...
		if ( WIFSTOPPED(status) ) {
			if ( WSTOPSIG(status) == SIGTRAP ) {		
				...
				assert( (instr_addr - 1 == break_main_addr) && "Something went wrong, debugger terminating");
