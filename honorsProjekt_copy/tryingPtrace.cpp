...
void find_print_brk_loc( string str, bool is_line_num ) {
	...
	if ( is_line_num ) {
		...
		if ( (it = line_to_addr.find(line_num)) != line_to_addr.end() ) {
			if ( initial_brkpt.find(it -> second) == initial_brkpt.end() ) {
				++brkpt_num;
				cerr << "Breakpoint " << brkpt_num << " set at address: '" << it -> second << "', file: '";
				cerr << filename << "', line: '" << line_num << "'\n";
				initial_brkpt[it -> second] = brkpt_num;
			}
			else {
				cerr << "Breakpoint already set at this address, breakpoint request ignored\n";
			}
...
void find_print_rm_loc( string str, bool is_line_num ) {
	if ( is_line_num ) {
		...
		if ( (line_it = line_to_addr.find(line_num)) != line_to_addr.end() ) {
			auto it = initial_brkpt.begin();
		
			if ( (it = initial_brkpt.find(line_it -> second)) != initial_brkpt.end() ) {
				cerr << "Removing breakpoint at line: '" << line_num << "'\n";
				initial_brkpt.erase( it );				// delete
			}
			else {
...
void handle_brk_rm( string input_cmd, bool in_break ) {
...
void print_brkpt_info() {
	if ( !initial_brkpt.size() ) {
		cerr << "No active breakpoints are set\n";
	}
	else {
		cerr << "  Number\tType\t\tAddress\t\tLocation\n";
		
		for ( auto it = initial_brkpt.begin(); it != initial_brkpt.end(); ++it ) {
...
void prog_init( pid_t pid ) {
	...
	while ( getline(cin, input_cmd) ) {
		if ( !input_cmd.compare("run") || !input_cmd.compare("r") ) {
			...
			handle_brk_rm( input_cmd, true );
		}
		else if ( input_rm(input_cmd) ) {
			handle_brk_rm( input_cmd, false );
		}
...
void prog_active( pid_t pid ) {
	if ( is_brkpt(pid) ) {
		...
		cerr << "Breakpoint " << initial_brkpt[new_instr_addr] << ", at " << filename << ":" << line_num << "\n";
	...
	if ( cont_flag )
		ptrace( PTRACE_CONT, pid, NULL, NULL );
	else ptrace( PTRACE_SINGLESTEP, pid, NULL, NULL );
}
	...
	if ( parent ) {
		...
		do {
			...
			if ( WIFEXITED(status) || (WIFSIGNALED(status) && WTERMSIG(status) == SIGKILL) ) {
				//cerr << "Process " << parent << " terminated\n";
				break;
			}
			...
			if ( WIFSTOPPED(status) ) 
				prog_active( parent );
/*
			cerr << "Child stopped on address: " << get_instr_ptr( parent ) << "\n";

			if ( flag ) {
				// might want to set all breakpoints
				cerr << "Resetting breakpoint to previous " << all_brkpts[brkpt_pos].orig_addr << " \n";
				...
				if ( is_brkpt(parent) ) {
					cerr << "\n--------------------------------------------------------------------------------\n\n";
					cerr << "In breakpoint: " << get_instr_ptr( parent ) << "\n";
					...
					cerr << "In breakpoint after reset data: " << get_instr_ptr( parent ) << "\n";
