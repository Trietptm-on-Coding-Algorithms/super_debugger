...
void prog_init( pid_t pid ) {
	...
	while ( getline(cin, input_cmd) ) {
		if ( !input_cmd.compare("run") || !input_cmd.compare("r") ) {
			...
			// kill program
			// might want an are you sure option
			if ( confirm_exit() ) {
				kill( parent, SIGKILL );
				break;
			}
			...
			if ( isdigit(input_cmd[0]) ) {
				if ( isNum(input_cmd) ) {
					...
					//initial_brkpt.push_back( find_addr(input_cmd, 1) );
				}
			}
			else {
				if ( isValidFunc(input_cmd) ) {
					cerr << "Breakpoint set at: '" << input_cmd << "'\n";
					//initial_brkpt.push_back( find_addr(input_cmd, 0) );
				}
			}
		}
...
*/
	...
	for ( int i = 0; i < initial_brkpt.size(); ++i ) 
		set_brkpt( parent, initial_brkpt[i] );
